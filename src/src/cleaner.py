import json, re, argparse, pandas as pd, numpy as np, os

def to_float(s):
    s=str(s).replace("\u00a0"," ").replace(",",".")
    s=re.sub(r"[^\d.]", "", s); m=re.findall(r"\d+(?:\.\d+)?", s)
...
def rooms_n(s):
    m=re.search(r"(\d+)\s*pi(?:è|e)ce", str(s), re.I)
    return to_float(m.group(1)) if m else np.nan

def main(inp, outp):
    with open(inp, "r", encoding="utf-8") as f: raw=json.load(f)
    df=pd.DataFrame(raw)
